...
export default function Invoice() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:5000/api/orders/order-history/order/${orderId}`)
  ...
  const products = orderHistory.map(item => ({
    ...
    image: `http://localhost:5000/uploads/${item.product_variant_image}`, // adjust path
    ...
    <div className="s-py-100 min-h-screen">
      <div className="container">
        <div className="max-w-[800px] mx-auto">
          ...
          {/* Customer + Shop Info */}
          <div className="grid md:grid-cols-2 gap-5 mb-10 bg-invoice p-6 rounded-xl border">
            <div>
          ...
          <div className="mt-6 text-right">
            <p>Sub Total: ₹{orderInfo.totalAmount}</p>
            {orderInfo.couponCodeName && (
              <p>
                Coupon ({orderInfo.couponCodeName}): -₹
                {orderInfo.totalAmount - orderInfo.grandTotalAmount}
              </p>
            )}
